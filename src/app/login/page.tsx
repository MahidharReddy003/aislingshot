    ...
    try {
      ...
      if (pendingCredential) {
        ...
        toast({
          ...
          description: 'Your Google account is now linked to your email/password login.',
        });
      }
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (error.code === 'auth/account-exists-with-different-credential') {
        // This is the account linking trigger
        ...
        toast({
          ...
          description: 'We found an existing account with this email. Linking sign-in methods.',
        });
      } else {
        toast({
    ...
    <div className="flex min-h-[80vh] items-center justify-center px-4 py-12">
      <Card className="w-full max-w-md shadow-xl border-2">
        <CardHeader className="space-y-1 text-center">
          ...
          <CardDescription>
            Enter your credentials to access your personalized hub.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          {showLinkingPrompt && (
            <Alert variant="default" className="bg-primary/5 border-primary/20">
              ...
              <AlertTitle className="text-primary font-bold">Account Linking Needed</AlertTitle>
              <AlertDescription className="text-xs">
                We found an existing account for <strong>{linkingEmail}</strong>. 
                Please enter your password to link Google to this account.
              </AlertDescription>
            </Alert>
          )}
          ...
          <form onSubmit={handleEmailLogin} className="space-y-4">
            ...
            <Button className="w-full font-bold" type="submit" disabled={loading}>
              ...
              {showLinkingPrompt ? 'Sign In & Link Google' : 'Sign In'}
            </Button>
          </form>
          ...
          {showLinkingPrompt && (
            <Button variant="ghost" className="w-full text-xs" onClick={() => setShowLinkingPrompt(false)}>
              Cancel Linking
            </Button>
          )}
        ...
        <CardFooter className="flex flex-col space-y-4">
          ...
          <p className="text-center text-[10px] text-muted-foreground opacity-70 italic">
            "We don't sell or share your data. Your trust is our priority."
          </p>
        </CardFooter>
      </Card>
    </div>
